...
var _KubeJSPaths = null;
var _CACHE_PATH = null;
try {
    _KubeJSPaths = Java.loadClass("dev.latvian.mods.kubejs.KubeJSPaths");
    var _localCache = _KubeJSPaths.LOCAL_CACHE;
    _CACHE_PATH = _localCache.getFileSystem().getPath(String(_localCache) + "/auto_tags.json");
    console.log("[auto_tags] cache path: " + _CACHE_PATH);
} catch (e) { console.log("[auto_tags] init error: " + e); }
...
function readCache() {
    if (!_CACHE_PATH) return null;
    try { return JSON.parse(JsonIO.readString(_CACHE_PATH)); } catch (e) { return null; }
}

function writeCache(data) {
    if (!_CACHE_PATH) { console.log("[auto_tags] writeCache: path is null"); return; }
    try {
        _KubeJSPaths.dir(_CACHE_PATH.getParent(), true);
        JsonIO.write(_CACHE_PATH, JsonIO.parseRaw(JSON.stringify(data)).getAsJsonObject());
        console.log("[auto_tags] cache written ok");
    } catch (e) { console.log("[auto_tags] writeCache error: " + e); }
}
...
ServerEvents.tags("item", function (event) {
    ...
    var cacheKey = quickHash(
        JSON.stringify(taxonomy) + JSON.stringify(materials) +
        JSON.stringify(modifierTokens) + JSON.stringify(standaloneTags) +
        JSON.stringify(customEmiGroups) + JSON.stringify(nativeEmiGroups) +
        String(allItemIds.length)
    );
    ...
    if (cached && cached.key === cacheKey) {
        ...
        for (var _ck in ct) {
            var _cv = ct[_ck];
            for (var _ci = 0; _ci < _cv.length; _ci++) {
                event.add(_ck, _cv[_ci]);
            }
        }
        return;
    }
    ...
    var tagAssignments = {};
    function recordTag(tag, id) {
        if (!tagAssignments[tag]) tagAssignments[tag] = [];
        tagAssignments[tag].push(id);
        event.add(tag, id);
    }
        ...
        for (var _i = 0; _i < allItemIds.length; _i++) {
            ...
            if (useMods || useMats) {
                ...
                if (fingerprint.length > 0 || material) {
                    ...
                    parts.push(key);
                    recordTag("kubejs:" + parts.join("_"), idStr);
                } else {
                    recordTag("kubejs:" + key, idStr);
                }
            } else {
                recordTag("kubejs:" + key, idStr);
            }
    ...
    for (var tagKey in tagNodeMap) {
        ...
        for (var ti = 0; ti < refs.length; ti++) {
            recordTag("kubejs:" + tagKey, "#" + refs[ti]);
        }
    ...
    writeCache({key: cacheKey, tags: tagAssignments});
});